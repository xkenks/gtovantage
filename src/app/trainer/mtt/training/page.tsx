...
function MTTTrainingPageContent() {
  ...
  const generateNewScenario = () => {
    ...
    if (actionType === 'vsopen') {
      ...
      if (urlOpener && isValidVsOpenCombination(effectivePosition, urlOpener)) {
        openerPosition = urlOpener;
      } else {
        // 指定されていない、または無効な場合はランダムに選択
        const validOpeners = getValidOpenerPositions(effectivePosition);
    ...
    if (stackSize === '30BB') {
      ...
      if (actionType === 'vsopen' && openerPosition === 'BTN' && stackSize === '15BB') {
        ...
        if (stackSize === '15BB') {
          threeBetSize = 15; // 15BBの3ベットはオールイン
          potSize = openRaiseSize + threeBetSize + 0.5; // オープン + 3ベット + SB残り
        } else {
          potSize = 13;
        }
    ...
    // 3ベッターのポジションを事前に計算
    const threeBetterPos = actionType === 'vs3bet' ? (() => {
      // vs 3ベットでは、ヒーロー以外のランダムなポジションを3ベッターとして選択
      const allPositions = ['UTG', 'UTG1', 'LJ', 'HJ', 'CO', 'BTN', 'SB', 'BB'];
      const availablePositions = allPositions.filter(pos => pos !== position);
      const selectedPos = availablePositions[Math.floor(Math.random() * availablePositions.length)];
      console.log('🎯 3ベッター選択:', { heroPosition: position, threeBetterPosition: selectedPos, actionType, stackSize });
      return selectedPos;
    })() : 
    actionType === 'vs4bet' ? position : undefined;

    // 3ベッターのスタックを計算する関数
    const getStackForPosition = (pos: string) => {
      if (actionType === 'vs3bet' && stackSize === '15BB' && threeBetterPos === pos) {
        console.log(`🎯 ${pos}のスタックを0に設定（3ベッター）`);
        return 0;
      }
      if (actionType === 'vs4bet' && openerPosition === pos) {
        return 0;
      }
      if (pos === 'SB') {
        return parseInt(stackSize) - 0.5;
      }
      if (pos === 'BB') {
        return parseInt(stackSize) - 1;
      }
      return parseInt(stackSize);
    };

    // 3ベッターのスタックを強制的に0に設定
    if (actionType === 'vs3bet' && stackSize === '15BB' && threeBetterPos) {
      console.log(`🚨 強制設定: ${threeBetterPos}のスタックを0に設定`);
    }
    ...
    const newSpot: Spot = {
      ...
      // vs3bet用の追加情報
      threeBetSize: (stackSize === '30BB' || stackSize === '15BB') && (actionType === 'vs3bet' || actionType === 'vs4bet') ? threeBetSize : undefined,
      threeBetterPosition: threeBetterPos,
      // 各ポジションのスタック情報を作成（全てのポジションに同じスタックを設定）
      positions: {
        'UTG': { active: true, stack: actionType === 'vs3bet' && stackSize === '15BB' && threeBetterPos === 'UTG' ? 0 : parseInt(stackSize), isHero: position === 'UTG' },
        'UTG1': { active: true, stack: actionType === 'vs3bet' && stackSize === '15BB' && threeBetterPos === 'UTG1' ? 0 : parseInt(stackSize), isHero: position === 'UTG1' },
        'LJ': { active: true, stack: actionType === 'vs3bet' && stackSize === '15BB' && threeBetterPos === 'LJ' ? 0 : parseInt(stackSize), isHero: position === 'LJ' },
        'HJ': { active: true, stack: actionType === 'vs3bet' && stackSize === '15BB' && threeBetterPos === 'HJ' ? 0 : parseInt(stackSize), isHero: position === 'HJ' },
        'CO': { active: true, stack: actionType === 'vs3bet' && stackSize === '15BB' && threeBetterPos === 'CO' ? 0 : parseInt(stackSize), isHero: position === 'CO' },
        'BTN': { active: true, stack: actionType === 'vs3bet' && stackSize === '15BB' && threeBetterPos === 'BTN' ? 0 : parseInt(stackSize), isHero: position === 'BTN' },
        'SB': { active: true, stack: actionType === 'vs3bet' && stackSize === '15BB' && threeBetterPos === 'SB' ? 0 : (parseInt(stackSize) - 0.5), isHero: position === 'SB' },
        'BB': { active: true, stack: actionType === 'vs3bet' && stackSize === '15BB' && threeBetterPos === 'BB' ? 0 : (parseInt(stackSize) - 1), isHero: position === 'BB' }
      }
    };

    // 3ベッターのスタックを強制的に0に設定（positions作成後）
    if (actionType === 'vs3bet' && stackSize === '15BB' && threeBetterPos) {
      // 直接スタックを0に設定
      if (newSpot.positions && newSpot.positions[threeBetterPos]) {
        newSpot.positions[threeBetterPos].stack = 0;
        console.log(`✅ 強制設定完了: ${threeBetterPos}のスタックを0に設定しました`);
      }
      
      // 全ポジションのスタックを確認
      console.log('🔍 全ポジションのスタック確認:', {
        threeBetterPos,
        'UTG': newSpot.positions?.UTG?.stack,
        'UTG1': newSpot.positions?.UTG1?.stack,
        'LJ': newSpot.positions?.LJ?.stack,
        'HJ': newSpot.positions?.HJ?.stack,
        'CO': newSpot.positions?.CO?.stack,
        'BTN': newSpot.positions?.BTN?.stack,
        'SB': newSpot.positions?.SB?.stack,
        'BB': newSpot.positions?.BB?.stack
      });
      
      // 最終確認: 3ベッターのスタックが0になっているかチェック
      if (newSpot.positions && newSpot.positions[threeBetterPos]) {
        console.log(`🎯 最終確認: ${threeBetterPos}のスタック = ${newSpot.positions[threeBetterPos].stack}`);
      }
    }
    
    // デバッグログを追加
    if (actionType === 'vs3bet' && stackSize === '15BB') {
      console.log('🔍 15BB vs 3ベット デバッグ:', {
        actionType,
        stackSize,
        openerPosition,
        threeBetterPosition: newSpot.threeBetterPosition,
        threeBetSize: newSpot.threeBetSize,
        heroPosition: position,
        threeBetterPos,
        positions: newSpot.positions,
        'UTG stack': newSpot.positions?.UTG?.stack,
        'UTG1 stack': newSpot.positions?.UTG1?.stack,
        'LJ stack': newSpot.positions?.LJ?.stack,
        'HJ stack': newSpot.positions?.HJ?.stack,
        'CO stack': newSpot.positions?.CO?.stack,
        'BTN stack': newSpot.positions?.BTN?.stack,
        'SB stack': newSpot.positions?.SB?.stack,
        'BB stack': newSpot.positions?.BB?.stack
      });
    }
  ...
  // 統計データを更新する関数
  const updateTrainingStats = (isCorrect: boolean) => {
    if (typeof window !== 'undefined') {
      try {
        ...
        if (isCorrect) {
          ...
          stats.monthlyStats.consecutiveCorrect += 1;
        } else {
          stats.incorrectAnswers += 1;
          stats.monthlyStats.consecutiveCorrect = 0;
        }

        // 正解率を計算
        stats.accuracyRate = Math.round((stats.correctAnswers / stats.totalQuestions) * 100);
        ...
        // 月別正解率を更新
        const monthIndex = stats.monthlyAccuracy.findIndex((item: { month: string; rate: number; color: string }) => item.month === currentMonthName);
        if (monthIndex !== -1) {
          // 既存の月のデータがある場合、平均を計算
          const monthData = stats.monthlyAccuracy[monthIndex];
          const totalCorrectThisMonth = (monthData.rate * (stats.totalQuestions - 1) / 100) + (isCorrect ? 1 : 0);
          const newRate = Math.round((totalCorrectThisMonth / stats.totalQuestions) * 100);
          stats.monthlyAccuracy[monthIndex] = {
            month: currentMonthName,
            rate: newRate,
            color: newRate >= 80 ? 'bg-green-500' : newRate >= 60 ? 'bg-blue-500' : 'bg-orange-500'
          };
        }

        // 今月の統計を更新
        stats.monthlyStats.totalPractice += 1;
        stats.monthlyStats.averageAccuracy = stats.accuracyRate;

        // ローカルストレージに保存
        localStorage.setItem('trainingStats', JSON.stringify(stats));
      } catch (error) {
        console.error('統計データの更新に失敗しました:', error);
      }
    }
  };
  ...
  const handleActionSelect = (action: string) => {
    ...
    // RFI、MIN、RAISEをすべてRAISEとして扱う
    let correct = false;
    if ((correctBase === 'RFI' || correctBase === 'MIN') && selectedBase === 'RAISE') {
      correct = true;
    } else if (selectedBase === correctBase) {
      correct = true;
    }
                    ...
                    <div>
                      ...
                      {actionType === 'vs3bet' && gtoData.openRaiserPosition && (
                        <div className={`${isMobile ? 'bg-gray-700/10' : 'bg-gray-700/30'} p-3 rounded mb-4`}>
                          ...
                          <div className="flex justify-between text-sm mt-1">
                            <span className="text-gray-400">3ベット({gtoData.threeBetterPosition || 'BTN'}):</span>
                            <span className="font-medium">{stackSize}</span>
                          </div>
                          <div className="flex justify-between text-sm mt-1">
                            <span className="text-gray-400">現在のポット:</span>
                          ...
                          <div className="text-xs text-gray-300 space-y-1">
                            ...
                            {actionType === 'vs3bet' && gtoData.openRaiserPosition && (
                              <>
                                ...
                                <div className="flex justify-between">
                                  <span>3ベット({gtoData.threeBetterPosition || 'BTN'}):</span>
                      ...
                      {/* 15BBスタック固有のベットサイズ情報 */}
                      {stackSize === '15BB' && gtoData && !gtoData.isInvalidCombination && (
                        <div className="bg-orange-900/20 p-3 rounded border border-orange-600/30 mb-4">
                          <h5 className="text-orange-300 font-semibold text-xs mb-2">🎯 15BBベットサイズ</h5>
                          <div className="text-xs text-gray-300 space-y-1">
                            {actionType === 'openraise' && (
                              <div className="flex justify-between">
                                <span>オープンレイズ:</span>
                                <span className="font-semibold text-orange-400">2.1BB</span>
                              </div>
                            )}
                            {actionType === 'vsopen' && (
                              <div className="flex justify-between">
                            ...
                            {actionType === 'vs3bet' && gtoData.openRaiserPosition && (
                              <>
                                ...
                                <div className="flex justify-between">
                                  <span>3ベット({gtoData.threeBetterPosition || 'BTN'}):</span>
                                  <span className="font-semibold text-red-400">15BB</span>
                                </div>
                                <div className="flex justify-between">
                                  <span>現在のポット:</span>
                                  <span className="font-semibold text-orange-400">{spot?.potSize}BB</span>
                                </div>
                              </>
                            )}
                          </div>
